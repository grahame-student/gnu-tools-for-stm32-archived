...
# Copy only files needed for newlib stage
# This improves Docker layer caching - changes to later-stage sources (gdb)
# won't invalidate the newlib layer cache

# Copy build script required for newlib
COPY build-newlib.sh /root/build/gnu-tools-for-stm32/

# Copy only newlib sources (prerequisites, binutils, and gcc already built in earlier stages)
# Excluding later-stage sources (gdb) - they're not needed until later stages
...
#
# Component built:
# 1. Newlib (3.3.0): C standard library for embedded systems (uses autoconf 2.69)
#                    Autogenerated files tracked: 15 (hand-crafted Makefile.in in libgloss subdirectories)
#                    Libraries: libc.a, libm.a, libg.a, crt0.o startup files
#                    Built with -Os for size optimization
#
# The regenerate_autotools() function (build-common.sh) uses autoconf 2.69 for newlib
# (exact version required for reproducible builds).
WORKDIR /root/build/gnu-tools-for-stm32
RUN echo "=== Newlib Stage: Building newlib C standard library ===" && \
    ...
    ./build-newlib.sh && \
    echo "=== Housekeeping: Cleaning up build artifacts ===" && \
    # Build artifacts already cleaned by build-newlib.sh:
...
# won't invalidate the gcc-final-gdb layer cache
...
#
# Components built:
# 1. GCC Final Pass:    C and C++ compiler with full standard library support (uses autoconf 2.69)
#                       Autogenerated files tracked: 15 (hand-crafted Makefile.in)
#                       Libraries: libstdc++.a, libsupc++.a
#                       Built with C++ support enabled (C compiler was built in binutils-gcc-first)
# 2. GDB (10.2):        GNU Debugger for ARM targets (uses autoconf 2.69)
#                       Autogenerated files tracked: 95 (most of any component)
#                       Tools: arm-none-eabi-gdb, arm-none-eabi-gdb-add-index
#
# The regenerate_autotools() function (build-common.sh) uses autoconf 2.69 for gcc and gdb
# (exact version required for reproducible builds).
WORKDIR /root/build/gnu-tools-for-stm32
RUN echo "=== GCC-Final-GDB Stage: Building GCC final pass and GDB ===" && \
    ...
    echo "=== Housekeeping: Cleaning up build artifacts ===" && \
    # Build artifacts already cleaned by build-gcc-final-gdb.sh:
