...
FILE *cyclomatic_complexity_file = NULL;
...
/* Output cyclomatic complexity information.  */
void
output_cyclomatic_complexity (void)
{
  if (cyclomatic_complexity_file)
    {
      /*
       * Homepage:
       ...
       */
      int complexity = n_edges_for_fn(cfun) - n_basic_blocks_for_fn(cfun) + 2;

      print_decl_identifier (cyclomatic_complexity_file, current_function_decl,
			     PRINT_DECL_ORIGIN | PRINT_DECL_NAME);
      fprintf (cyclomatic_complexity_file, "\t" HOST_WIDE_INT_PRINT_DEC "\n",
	       complexity);
    }
}
...
lang_dependent_init (const char *name)
{
  ...
  if (!flag_wpa)
      ...
      if (!flag_generate_lto && !flag_compare_debug)
	{
	  ...
	  /* If cyclomatic complexity information is desired, open the output
	     file.  */
	  if (flag_cyclomatic_complexity)
	    cyclomatic_complexity_file = open_auxiliary_file ("cyclo");
	}
      else
...
finalize (bool no_backend)
{
  ...
  if (cyclomatic_complexity_file)
    {
      fclose (cyclomatic_complexity_file);
      cyclomatic_complexity_file = NULL;
    }
