...
static wchar_t *
argv_to_cmdline (wchar_t *const *argv)
{
  wchar_t *cmdline;
  wchar_t *p;
  size_t cmdline_len;
  ...
  cmdline = XNEWVEC (wchar_t, cmdline_len);
  p = cmdline;
  for (i = 0; argv[i]; i++)
    {
      needs_quotes = 0;
	...
	*e++ = '\\';
      ...
      for (ext = std_suffixes; *ext; ext++)
	{
	  ...
	  *e = '\0';
	  ...
	  wfull_executable = handle_long_path_utf8 (full_executable);
	  if (!wfull_executable)
	    continue;

	  /* Attempt to open this file.  */
	  h = CreateFileW (wfull_executable, GENERIC_READ,
	     ...
	     LPPROCESS_INFORMATION pi)
{
  ...
  if (!wfull_executable)
    goto exit;
  new_argc = argv_to_argc(argv);
  norm_argv = XNEWVEC (wchar_t *, new_argc + 1);
  if (!norm_argv)
    {
      new_argc = 0;
      goto exit;
  ...
  norm_argv[0] = handle_long_path_utf8 (argv[0]);
  if (!norm_argv[0])
    {
      new_argc = 1;
      goto exit;
  ...
  for (int i = 1; i < new_argc; i++)
    {
      norm_argv[i] = utf8_to_wchar (argv[i]);
      if (!norm_argv[i])
       {
         new_argc = i;
         goto exit;
  ...
  norm_argv[new_argc] = NULL;
  cmdline = argv_to_cmdline (norm_argv);
  if (!cmdline)
    goto exit;
  ...
  if (wcslen (cmdline) > 32767)
    {
      ...
      wchar_t *wresponse_arg = utf8_to_wchar (response_arg);
      free (response_arg);
      if (!wresponse_arg)
        goto exit;
      wchar_t *wresponse_argv[3] = {norm_argv[0], wresponse_arg, NULL};
      free (cmdline);
      cmdline = argv_to_cmdline (wresponse_argv);
      free (wresponse_arg);
      if (!cmdline)
        goto exit;
    }
  ...
  if (CreateProcessW (wfull_executable, cmdline,
